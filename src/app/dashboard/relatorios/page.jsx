...
export default function RelatoriosPage() {
  ...
  return (
    <div className="animate-in fade-in duration-700 space-y-8 h-screen flex flex-col p-4 md:p-8 overflow-hidden bg-slate-50/50">
          ...
          <div className="bg-white p-4 rounded-[2.5rem] border border-slate-200/60 shadow-sm flex flex-col h-full overflow-hidden">
            ...
            <div className="relative mb-6 shrink-0">
              ...
              <input 
                type="date" 
                value={filtroData}
                className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-3.5 pl-12 pr-4 text-sm font-bold text-slate-600 outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-400 transition-all"
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      <div className={`shrink-0 p-2.5 rounded-xl ${
                        ...
                        : rel.turnoIcone === "sol" ? "bg-amber-50 text-amber-500" : "bg-indigo-50 text-indigo-500"
                      }`}>
                        {rel.turnoIcone === "sol" ? <Sun size={20} /> : <Moon size={20} />}
                      </div>
                      ...
                      <div className="flex flex-col overflow-hidden min-w-0">
                        ...
                        <p className="text-sm font-bold truncate leading-tight">
                          {rel.responsavel?.split('-')[0] || "Sem Responsável"}
                        </p>
                        <div className="flex items-center gap-1.5 mt-0.5">
                           <p className={`text-[9px] font-mono truncate ${relatorioSelecionado?.id === rel.id ? "text-blue-200" : "text-slate-300"}`}>
                            ID: {rel.hash?.substring(0,12) || "---"}
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            <div className="flex flex-col h-full bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
              ...
              <div className="bg-slate-50 border-b border-slate-100 px-8 py-4 flex justify-between items-center shrink-0">
                <div className="flex items-center gap-3">
                  <div className={`w-2.5 h-2.5 rounded-full animate-pulse ${relatorioSelecionado.turnoIcone === 'sol' ? 'bg-amber-400' : 'bg-indigo-400'}`} />
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">
                      Conferência {relatorioSelecionado.turnoIcone === 'sol' ? 'Diurna' : 'Noturna'}
                    </p>
                    <p className="text-xs font-bold text-slate-700 mt-1">Autenticado pelo 17º BPM</p>
                  </div>
                </div>
                ...
                <SecondaryButton 
                  ...
                  onClick={handleOpenNewTab}
                  className="min-h-[38px]!py-2! text-[11px]"
                />
              </div>
              ...
              <div className="flex-1 bg-slate-800 relative">
                {relatorioSelecionado.pdfUrl ? (
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-12 text-center bg-white">
                    ...
                    <h3 className="text-slate-800 font-bold text-lg">Arquivo não disponível</h3>
                    <p className="text-slate-500 text-sm max-w-xs mt-2">
                      O registro existe, mas o link do arquivo PDF não foi localizado.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white border-t border-slate-100 p-4 px-8 flex justify-between items-center shrink-0">
                <p className="text-[9px] font-mono text-slate-400 truncate max-w-[60%]">
                  HASH: {relatorioSelecionado.hash}
                </p>
                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-tighter">
                  SISTEMA DE FURRIELAÇÃO • 17º BPM
                </p>
              </div>

            </div>
          ) : (
            <div className="h-full flex flex-col items-center justify-center text-slate-300 border-2 border-dashed border-slate-200 rounded-[3rem] bg-slate-50/50 p-8">
                ...
                <p className="font-black uppercase tracking-[0.2em] text-sm text-slate-400">
                  {isLoading ? "Consultando Banco Neon..." : "Selecione um documento"}
                </p>
            </div>
          )}
        </div>
